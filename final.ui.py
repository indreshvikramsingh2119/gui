...
class SleepSensePlot(QMainWindow):
    ...
    def init_ui(self):
        ...
        right_layout.addWidget(self.spo2_label)
        ...
        self.canvas = FigureCanvas(Figure(figsize=(98, 66)))  
    ...
    #         # Debug print - Check data values and lengths
    #         print(" SPO2 Raw (masked):", spo2[:10])
    #         print(" Time (t):", t[:10])
    #         print(" Offset:", offset.get('SpO2', 0))
    #         print(" Length SPO2:", len(spo2))
    #         print(" Length t:", len(t))

    #         # Step 2: Rolling average (smoothing)
    #         if len(spo2) > window_size:
    #             spo2 = pd.Series(spo2).rolling(window=window_size, min_periods=1, center=True).mean()

    #         # Step 3: Match length of time and spo2
    #         if len(t) != len(spo2):
    #             min_len = min(len(t), len(spo2))
    #             t = t.iloc[:min_len]
    #             spo2 = spo2.iloc[:min_len] if isinstance(spo2, pd.Series) else spo2[:min_len]

    #         t_segment = t.reset_index(drop=True)  # Reset index for plotting

    #         # Step 4: Plot line
    #         self.ax.plot(t, spo2 + offset['SpO2'], label="SpO2", color="green", linewidth=0.5)

    #         # Step 5: Show value on graph every 30th point
    #         for i in range(0, len(spo2), 30):
    #             x = t_segment.iloc[i]
    #             y = spo2.iloc[i]
    #             self.ax.text(
    #                 x, y + offset['SpO2'],
    #                 f"{y:.0f}",  # Rounded SpO2 value
    ...
    #             )
        ...
        if self.visible_signals.get('SpO2', False):
            ...
            # Plot the SpO2 wave (optional, remove if you only want numbers)
            self.ax.plot(t, spo2 + offset['SpO2'], label="SpO2", color="green", linewidth=2.0)
            self.ax.set_ylim(-0.5, max(spo2) + offset['SpO2'] + 5)
            # Show numeric value every 10th point
            for i in range(0, len(spo2), 50):
                x = t_segment.iloc[i]
                y = offset['SpO2'] + 0.1  # Fixed band
                self.ax.text(
                    x, y,
